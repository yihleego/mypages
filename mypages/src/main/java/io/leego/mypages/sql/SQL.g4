...
queryOrganization
    : (ORDER BY order+=sortItem (',' order+=sortItem)*)?
      ...
      windowClause?
      (LIMIT (ALL | limit=limitExpression))?
    ;
...
limitExpression
    : (offset=INTEGER_VALUE ',')? rows=INTEGER_VALUE
    | rows=INTEGER_VALUE OFFSET offset=INTEGER_VALUE
    | booleanExpression
    ;
...
TRY_CAST: T R Y UNDERSCORE C A S T;
TYPE: T Y P E;
